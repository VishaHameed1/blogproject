...
<div class="max-w-4xl mx-auto animate-fade-in">
    ...
    <div class="mb-8 text-center sm:text-left">
        <h1 class="heading-font text-3xl font-bold text-[var(--color-text-primary)] tracking-tight">Edit Category</h1>
        <p class="text-sm text-[var(--color-text-muted)] mt-1">Update category details below</p>
    </div>

    {{-- Form --}}
    <form method="POST" action="{{ route('admin.categories.update', $category) }}" enctype="multipart/form-data" class="bg-[var(--color-bg-card)] border border-[var(--color-border]) rounded-2xl shadow-xl p-6 md:p-8 space-y-6 transition-all duration-300 hover:border-[var(--color-primary)]/30">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="px-4 py-3 rounded-xl bg-[var(--color-bg)] border border-[var(--color-border]) text-[var(--color-text-muted)] font-mono text-sm break-all">
                {{ $category->slug }}
            </div>
        ...
        @if($category->image)
        <div>
            <label class="block text-sm heading-font font-semibold text-[var(--color-text-secondary)] mb-2">Current Image</label>
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 bg-[var(--color-bg)] rounded-xl border border-[var(--color-border]) transition-opacity" id="current-image-container">
                <img src="{{ $category->image_url }}"
                    alt="{{ $category->name }}"
                    class="h-20 max-w-full w-auto rounded-xl object-cover border border-[var(--color-border]) shadow-sm">
        ...
        {{-- Upload New Image --}}
        <div>
            <label class="block text-sm heading-font font-semibold text-[var(--color-text-secondary)] mb-2">Upload New Image</label>
            <div class="w-full">
                <input type="file"
                    name="image"
                    id="image"
            ...
            <div id="image-preview" class="hidden mt-3">
                <p class="text-xs heading-font font-semibold text-[var(--color-text-muted)] mb-2">New Image Preview</p>
                <div class="relative inline-block">
                    <img id="preview" src="" alt="Preview" class="h-32 max-w-full w-auto rounded-xl border border-[var(--color-border]) object-cover shadow-md">
                </div>
            </div>
        </div>
        ...
        {{-- Post Count --}}
        <div class="p-4 bg-[var(--color-bg)] rounded-xl border border-[var(--color-border]) text-center sm:text-left">
            <p class="text-xs sm:text-sm text-[var(--color-text-secondary)]">
                <span class="heading-font font-semibold text-[var(--color-primary)]">{{ $category->posts()->count() }}</span>
