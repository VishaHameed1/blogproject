...
<div class="max-w-3xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="text-center sm:text-left">
        <h1 class="heading-font text-3xl font-bold text-[var(--color-text-primary)] tracking-tight">New User</h1>
        <p class="text-sm text-[var(--color-text-secondary)] mt-1">Create a new user account</p>
    </div>

    {{-- Form --}}
    <form method="POST" action="{{ route('admin.users.store') }}" class="bg-[var(--color-bg-card)] border border-[var(--color-border]) rounded-2xl shadow-xl p-6 md:p-8 space-y-6 transition-all duration-300 hover:border-[var(--color-primary)]/30">
        @csrf
        ...
        <div class="group">
            ...
            <input type="text"
                ...
                value="{{ old('name') }}"
                required
                class="w-full px-4 py-3 rounded-xl bg-[var(--color-bg)] border border-[var(--color-border)] text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]/20 transition-all"
                placeholder="John Doe">
            @error('name')
            <p class="text-red-500 dark:text-red-400 text-xs mt-1.5 body-font">{{ $message }}</p>
            @enderror
        </div>
        ...
        {{-- Password --}}
        <div class="group">
            <label for="password" class="block text-sm heading-font font-semibold text-[var(--color-text-secondary)] mb-2 transition-colors group-focus-within:text-[var(--color-primary)]">Password <span class="text-[var(--color-primary)]">*</span></label>
            <input type="password"
                name="password"
                id="password"
                required
        ...
        {{-- Confirm Password --}}
        <div class="group">
            <label for="password_confirmation" class="block text-sm heading-font font-semibold text-[var(--color-text-secondary)] mb-2 transition-colors group-focus-within:text-[var(--color-primary)]">Confirm Password <span class="text-[var(--color-primary)]">*</span></label>
            <input type="password"
        ...
        <div class="group">
            ...
            <select name="role_id"
                id="role_id"
                required
                class="w-full px-4 py-3 rounded-xl bg-[var(--color-bg)] border border-[var(--color-border]) text-sm text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]/20 transition-all appearance-none">
                <option value="" class="bg-[var(--color-bg-card)] text-[var(--color-text-muted)]">Select a role</option>
                @foreach($roles as $role)
                <option value="{{ $role->id }}" {{ old('role_id') == $role->id ? 'selected' : '' }} class="bg-[var(--color-bg-card)] text-[var(--color-text-primary)]">
                    {{ $role->name }} ({{ $role->slug }})
                </option>
                @endforeach
            </select>
