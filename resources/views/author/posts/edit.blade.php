...
<div class="max-w-5xl mx-auto py-6 sm:py-8 px-4 sm:px-6 body-font">
        ...
        <div class="p-6 sm:p-8">
            <form action="{{ route('author.posts.update', $post) }}"
                ...
                <div class="mb-5">
                    ...
                    <div class="relative">
                        <select name="category_id" id="category_id"
                            ...
                            <option value="{{ $category->id }}" class="bg-[var(--color-bg-card)]" {{ old('category_id', $post->category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-5">
                    ...
                    {{-- Image Preview --}}
                    <div x-show="imagePreview && !removeImage" x-transition class="mt-3 relative max-w-sm rounded-xl overflow-hidden border border-[var(--color-border)] bg-[var(--color-bg)]">
                        ...
                        <span x-show="!imageSelected" class="absolute bottom-2 left-2 text-[10px] bg-black/70 text-[var(--color-text-muted)] px-2 py-1 rounded-md body-font">Current</span>
                    </div>
                ...
                <div class="border-t border-[var(--color-border)] pt-5 flex items-center justify-end gap-3 heading-font">
                    <a href="{{ route('author.posts.index') }}"
                        class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] hover:bg-[var(--color-bg)] hover:border-[var(--color-primary)] text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] text-xs sm:text-sm font-semibold transition-all duration-300">
                        <span>Cancel</span>
                    </a>
    ...
    {{-- Post Status Info --}}
    <div class="mt-6 flex items-center justify-center gap-3 text-xs body-font">
        <span class="text-[var(--color-text-muted)]">Status:</span>
        <span class="px-2.5 py-1 rounded-full text-[10px] font-medium
            @if($post->status === 'published') bg-green-500/20 text-green-400 border border-green-500/20
            @elseif($post->status === 'pending') bg-[var(--color-primary-soft)] text-[var(--color-primary)] border border-[var(--color-primary-soft)]
            @elseif($post->status === 'rejected') bg-rose-500/20 text-rose-400 border border-rose-500/20
            @else bg-[var(--color-bg)] text-[var(--color-text-muted)] border border-[var(--color-border)] @endif">
            {{ ucfirst($post->status) }}
        </span>
        @if($post->rejection_reason)
        <span class="text-[var(--color-text-muted)]">|</span>
        <span class="text-rose-400/70 body-font">Feedback: {{ $post->rejection_reason }}</span>
        @endif
    </div>
