...
<style>
    ...
    /* Theme variables - Already defined in admin layout */
    /* Using var() for all colors */
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="text-center mb-8">
        <h1 class="heading-font text-2xl sm:text-3xl font-bold text-[var(--color-text-primary)] tracking-tight">Edit User</h1>
        <p class="text-sm text-[var(--color-text-muted)] mt-1">Update user details and role</p>
    </div>

    {{-- Form --}}
    <form method="POST" action="{{ route('admin.users.update', $user) }}" class="form-card border rounded-2xl shadow-xl p-6 md:p-8 transition-all duration-300">
        @csrf
        @method('PUT')
        ...
        {{-- Password --}}
        <div class="mb-6">
            <label for="password" class="form-label block text-sm font-medium mb-1.5 heading-font">Password</label>
            <input type="password"
                name="password"
                id="password"
                class="form-input w-full px-4 py-2.5 rounded-xl border placeholder:text-[var(--color-text-muted)] focus:border-[var(--color-primary)] focus:ring-1 focus:ring-[var(--color-primary)]/30 transition-all text-sm"
                placeholder="Leave blank to keep current password">
            <p class="helper-text text-xs mt-1">Minimum 8 characters. Leave blank to keep current password.</p>
            @error('password')
            <p class="error-message text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        {{-- Confirm Password --}}
        <div class="mb-6">
            <label for="password_confirmation" class="form-label block text-sm font-medium mb-1.5 heading-font">Confirm Password</label>
            <input type="password"
        ...
        <div class="mb-6">
            ...
            <select name="role_id"
                ...
                <option value="{{ $role->id }}" {{ old('role_id', $user->role_id) == $role->id ? 'selected' : '' }} class="text-[var(--color-text-primary)]">
                    {{ $role->name }} ({{ $role->slug }})
                </option>
                @endforeach
            </select>
